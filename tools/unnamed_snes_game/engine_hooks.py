...
from .json_formats import Name, EngineHookFunction, Mappings
...
class EngineHookType(NamedTuple):
    human_name: str
    parameter_array: str
    parameter_size: int


ROOM_EVENT_HOOK: Final = EngineHookType("room event", "room.roomEventParameters", 4)
...
def parse_hook_parameters(
    hook_type: EngineHookType,
    hook: EngineHookFunction,
    ...
    for p in hook.parameters:
        ...
        if p.type in AUTOGENERATED_PARAMETERS:
            if value:
                error_list.append(f"{ hook_type.human_name } parameter { p.name } is auto-generated and must not have a value")

            elif room_doors is None:
                error_list.append(f"{ hook_type.human_name } parameter { p.name } is auto-generated and there is no map data")
                ...
                if door_location is not None:
                    ...
                    error_list.append(f"{ hook_type.human_name } parameter {p.name}: Cannot find a locked door")
                ...
                error_list.append(f"Unknown { hook_type.human_name } parameter type: {p.type}")
        else:
            if not value:
                error_list.append(f"{ hook_type.human_name } {hook.name}: Missing parameter {p.name}")
            elif p.type == "u8":
                ...
                try:
                    ...
                    error_list.append(f"{ hook_type.human_name } parameter {p.name}: Cannot find gamestate flag: {value}")
            else:
                error_list.append(f"Unknown { hook_type.human_name } parameter type: {p.type}")

    if room_doors:
        # room_doors are only used by room events
        assert hook_type == ROOM_EVENT_HOOK
    ...
    if len(out) < hook_type.parameter_size:
        out += bytes(hook_type.parameter_size - len(out))

    if len(out) != hook_type.parameter_size:
        if len(error_list) == old_n_errors:
            # No errors with hook parameters, but out has the wrong size
            error_list.append("CRITICAL ERROR: { hook_type.human_name } data is too large")
        return bytes(hook_type.parameter_size)
    else:
        return out


def write_hook_parameters_wiz(out: StringIO, hooks: OrderedDict[Name, EngineHookFunction], hook_type: EngineHookType) -> None:
    for e in hooks.values():
        if e.parameters:
            ...
            for p in e.parameters:
                ...
                if not ptype:
                    raise ValueError(f"Unknown { hook_type.human_name } parameter type: {p.type}")

                if p.comment:
                    p_comment = p.comment.replace("\n", "\n  // ")
                    out.write(f"  // { p_comment }\n")
                out.write(f"  // ({ p.type })\n")
                out.write(f"  var parameter__{ p.name } @ &{ hook_type.parameter_array }[{ i }] : { ptype };\n")

                i += PARAM_SIZE[p.type]

            out.write("}\n\n")

            if i > hook_type.parameter_size:
                raise ValueError(f"{ hook_type.human_name } has too many parameters: {e.name}")
